...
> ✅ **Thai and Math are both live and both verified.** `ENABLED_SUBJECTS` lists both. The Math indicator text was machine-repaired after a font bug in the source PDF, and that repair is now proven clean by `npm run verify:math` — **0 failures**, including a negative test showing the verifier itself catches corruption (see §13.3). One honest limit remains: the check proves the repair *changed nothing it shouldn't*, not that every word is spelled correctly — a word missing from the `RESTORE` table stays as `า` and passes. So if the teacher reports a misspelling in a math indicator, that is expected residual risk, not a regression: fix the `RESTORE` table and re-run the script. Dropping `"math"` from that array is still the one-line rollback.
...
3. ✅ **Math indicator text — CLOSED 9 Aug 2569. Verified 0 failures, enabled in `ENABLED_SUBJECTS`.** The source `math.pdf` has a font bug that renders every สระ `า` as `ำ`, which left all 116 math indicators unreadable (`"หำค่ำของตัวไม่ทรำบค่ำ"` for `"หาค่าของตัวไม่ทราบค่า"`). The **codes were never affected**, and **Thai was never affected**. `scripts/fix_math_curriculum_text.mjs` repairs the text.

   **This was closed by team QA, not by the teacher — a deliberate decision.** Proofreading 109 lines is our work, not the domain expert's; her time is the scarcest resource on this project. `scripts/verify_math_text_repair.mjs` (`npm run verify:math`) replaces the proofread by proving mechanically that the repair only changed the characters it was designed to change:
   - Reads before/after straight from git history (`69fbaa1^` vs `69fbaa1`) — never re-parses the PDF, which is not in the repo.
   - Four checks per changed line: length unchanged / digit sequence unchanged (a moved number is a corrupted assessment criterion, not a font fix) / every differing position is exactly `ำ`↔`า` / every surviving `ำ` is explained by replaying the `RESTORE` table.
   - Result: **109/109 changed indicators pass, `strandText` 27/27 pass, Thai untouched, indicator codes untouched.** The 7 indicators the repair didn't touch are accounted for — 5 contain only genuine `ำ` (จำนวน/น้ำหนัก/จำแนก), 2 contain no `ำ` at all. None are unrepaired leftovers.
   - **The verifier is itself verified:** run against a fixture with 4 deliberately injected corruptions (dropped char, digit swap, unrelated letter swap, an unexplainable `ำ`), it caught all 4 and exited 1.

   ⚠️ **What this does not prove.** It shows the repair broke nothing; it does not show every word is spelled correctly. A word that *should* carry `ำ` but has no `RESTORE` entry comes out as `า` and passes every check, because that is exactly what the table dictates. Closing this item accepts that residual risk knowingly.
   - If the teacher reports a misspelled math indicator, that is expected residual risk, **not a regression** — add the word to the `RESTORE` table in the script and re-run it. **Never hand-edit `data/curriculum.json`** — the script overwrites it.
   - `docs/math-curriculum-review.md` still exists as the full before/after listing. It is now a reference for spot-checking, not a blocking deliverable.
   - If Math needs to be off for a real session, remove `"math"` from `ENABLED_SUBJECTS` — one line, Thai keeps working.
