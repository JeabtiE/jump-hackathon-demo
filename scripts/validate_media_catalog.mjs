 ...
 *               (parse_details() จับ block ไม่ติด) แก้ได้ต่อเมื่อมี pdfs/3.txt
...
for (const code of confirmed) {
  ...
  if ((item.disabilityTypes ?? []).includes("intellectual")) {
    ...
    gap(
      ...
      `(รอ pdfs/3.txt) | ครูยืนยันแล้วว่าใช้จริง — ห้ามเติมเองด้วยมือ`
    );
  } else {
    fail(
...
const tailNoise = [...catalog.values()].filter((i) =>
  /(?:ปรับคุณสมบัติ|ปรับราคา|ปรับรายการ|เพิ่มเติม|รายการใหม่)\s*$|\/\s*$/.test(i.name)
);
